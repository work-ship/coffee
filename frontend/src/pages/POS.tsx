    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-6 bg-neutral-50/50">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-extrabold text-2xl text-neutral-800 dark:text-white tracking-tight">
              Product Register
            </h1>
            <p className="text-xs text-neutral-400 font-semibold mt-0.5">Select coffees and snacks to build order</p>
          </div>
          <motion.span
            key={filteredProducts.length}
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="text-xs font-bold text-neutral-500 bg-white border border-neutral-200 shadow-sm px-3.5 py-2 rounded-full flex items-center gap-1.5"
          >
        ...
        <AnimatePresence mode="wait">
          {filteredProducts.length > 0 ? (
            ...
            >
              ...
              <p className="text-base font-bold text-neutral-600">No items match your filters</p>
              <p className="text-sm text-neutral-400 mt-1 max-w-[240px]">
